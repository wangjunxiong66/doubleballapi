...
public interface DoubleballCalculateMapper {
    ...
    // 通过id查询记录（从"计算双色球统计信息结果表doubleballcalculate"）
    @Select("select * from doubleballcalculate where id=#{id}")
    DoubleballCalculate getBallCalculateById(@Param("id") Integer id);

    // 查询最新的记录（从"计算双色球统计信息结果表doubleballcalculate"）
    @Select("select * from doubleballcalculate order by id desc limit 1")
    DoubleballCalculate getBallCalculateLimitOne();

    // 插入记录到"计算双色球统计信息结果表doubleballcalculate"
    ...
    // 通过id删除记录（从"计算双色球统计信息结果表doubleballcalculate"）
